/**
 * "Building Agentic AI Systems — Pre-Assessment" for the 2026-07-26 cohort.
 *
 * Identity fields (name, email, phone) + the 12 graded concept questions.
 * Nothing else — no self-rating scales, no open-ended paragraphs.
 *
 ...
 *
 * NOTE ON THE TWO EMAIL FIELDS: setCollectEmail(true) captures the signed-in
 * Google account as the "Username" column — that is the column the master-sheet
 * fill matches on. The explicit "Email" question below is the address the
 * trainee is registered under at the academy. These are often NOT the same
 * person's two addresses, and having both is what lets you resolve the
 * mismatches instead of guessing. Keep both.
 *
 * HOW TO RUN:
 ...
 *   5. Open View > Logs (or the Executions clock icon) for the URLs:
 ...
 * named "Building Agentic AI Systems — Pre-Assessment 2026-07-26 (Responses)".
 */

var COHORT = '2026-07-26';

function createPreAssessmentForm() {
  const form = FormApp.create('Building Agentic AI Systems — Pre-Assessment');
  form.setIsQuiz(true);
  form.setDescription(
    'A short pre-check before we start, so I can pitch the course at the ' +
    'right level. It is graded automatically, but it is a starting-point ' +
    'check — not an exam, and it does not affect your grade in the course. ' +
    'Answer what you can; guessing is fine.'
  );
  ...
  form.addTextItem()
    .setTitle('Full name')
    .setHelpText('As it appears on the academy roster.')
    .setRequired(true);

  const email = form.addTextItem()
    .setTitle('Email')
    .setHelpText('The email you are registered with at the academy.')
    .setRequired(true);
  email.setValidation(
  ...
  const phone = form.addTextItem()
    ...
    .setHelpText('Saudi mobile, e.g. 0501234567 or +966501234567.')
    .setRequired(true);
  phone.setValidation(
    FormApp.createTextValidation()
      .setHelpText('Enter a valid Saudi mobile number, e.g. 0501234567 or +966501234567.')
      .requireTextMatchesPattern('^(\\+?966|0)?5\\d{8}$')
      .build()
  );

  // ---- Graded concept questions (12 x 1 point) ----
  ...
  const ss = SpreadsheetApp.create(
    'Building Agentic AI Systems — Pre-Assessment ' + COHORT + ' (Responses)');
